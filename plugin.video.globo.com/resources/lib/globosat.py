...
class GlobosatApi(object):
    ...
    def _get_player_version(self):
        req = self._get_page(API_MIN_URL)
    ...
    def _get_hashes(self, video_id, resource_ids, auth_retry=False, player_retry=False):
        ...
        req = self._get_page(HASH_URL % args, cookies=_cookies)
    ...
    def _get_show_tree(self):
        # two trees shall be built, one based on categories, the other on channels
        tree = {}
    ...
    def authenticate(self):
        ...
        if not glbid and (username and password):
            ...
            if glbid:
                ...
                self.plugin.notify(self.plugin.get_string(32001))
        elif glbid:
